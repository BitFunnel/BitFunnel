...
namespace BitFunnel
{
    ...
    public:
        ...
        virtual RowId GetRowIdAdhoc(Term::Hash hash, unsigned rowOffset, unsigned variant)  const= 0;
        ...
        // by calling AddRowId().
        virtual void AddTerm(Term::Hash hash, unsigned rowIdOffset, unsigned rowIdLength) = 0;
        ...
        // the row table for (tier, rank). This includes rows allocated for 
        // facts, if applicable.
        virtual RowIndex GetTotalRowCount(Tier tier, Rank rank) const = 0;

        // Returns the number of rows associated with the mutable facts for 
        // RowTables with (tier, rank).
        virtual RowIndex GetMutableFactRowCount(Tier tier, Rank rank) const = 0;
